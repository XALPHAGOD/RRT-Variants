...
class RrtStar:
    def __init__(self, x_start, x_goal, step_len,
                 goal_sample_rate, search_radius, iter_max, max_nodes):
    ...
    def planning(self):
        ...
        for k in range(1,self.iter_max+1):
            ...
            if node_new and not self.utils.isCollisionOccurs(node_near, node_new):
                ...
                if len(self.vertex) >= self.max_nodes:
                    self.removeOne(node_near)
        ...
        if index != -1:
            ...
            if len(self.path) == 1:
                ...
                print("total no. of nodes ",len(self.vertex))

        else:
            print("No path found")

        self.plotting.animation(self.vertex, self.path, "RRT* Fixed Node, Iter= " + str(self.iter_max))

    def removeOne(self, node_near):
        childless_indices = self.removalEligibleNodes(node_near)  
        if len(childless_indices) > 0:
            rand_rem_index = random.choice(childless_indices)

            self.vertex[rand_rem_index].parent.children -= 1
            self.vertex.pop(rand_rem_index)

    def removalEligibleNodes(self,node_near):
        eligible_vertices = []
        for i in range(0,len(self.vertex)):
            if self.vertex[i] != self.s_start and self.vertex[i] != self. s_goal and self.vertex[i] != node_near and self.vertex[i].children <= 0 :
    ...
    def chooseParent(self, node_new, neighbor_index):
        ...
        if node_new.parent:
            node_new.parent.children -= 1
        node_new.parent = self.vertex[cost_min_index]
        self.vertex[cost_min_index].children += 1
    ...
    def rewire(self, node_new, neighbor_index):
        for i in neighbor_index:
            ...
            if self.cost(node_neighbor) > self.getNewCost(node_new, node_neighbor):
                if node_neighbor.parent:
                    node_neighbor.parent.children -= 1
...
def main():
    ...
    rrt_star = RrtStar(x_start, x_goal, 0.5, 0.10, 10, 10000, 2000)
    rrt_star.planning()

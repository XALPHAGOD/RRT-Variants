...
class QuickRrtStar:
    def __init__(self, x_start, x_goal, step_len,
                 goal_sample_rate, search_radius, iter_max, ancestry_depth = 2):
    ...
    def planning(self):
        ...
        self.plotting.animation(self.vertex, self.path, "Quick RRT*, Iter= " + str(self.iter_max))
    ...
    def chooseParent(self, node_new, neighbor_index):
        ...
        for i in neighbor_index:
            node = self.vertex[i]
            depth = self.ancestry_depth

            last_collision_free = node
            while node is not None and depth > 0:
                depth -= 1
                if not self.utils.isCollisionOccurs(node_new,node):
                    last_collision_free = node

                node = node.parent

            ancestors.append(last_collision_free)
            
        cost = [self.getNewCost(node, node_new) for node in ancestors]
        node_new.parent = ancestors[np.argmin(cost)]
    ...
    def rewire(self, node_new, neighbor_index):
        for i in neighbor_index:
            ...
            while node is not None:
                if self.cost(node_neighbor) > self.getNewCost(node_new, node_neighbor) and not self.utils.isCollisionOccurs(node_new,node_neighbor):
...
def main():
    ...
    rrt_star = QuickRrtStar(x_start, x_goal, 0.5, 0.10, 10, 10000)
    rrt_star.planning()
